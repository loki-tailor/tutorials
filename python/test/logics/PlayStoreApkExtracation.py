...
"""
    ...
    def long_press(self, x, y, time_in_seconds=2, refreshDump=False):
        ...
        if self.deviceId:
            self.cmdShell(
                [self.adbExe, "-s", self.deviceId, "shell", "input", "touchscreen", "swipe", str(x), str(y), str(x),
                 str(y), str(time_in_seconds * 1000)])
        else:
            self.cmdShell([self.adbExe, "shell", "input", "touchscreen", "swipe", str(x), str(y), str(x), str(y),
                           str(time_in_seconds * 1000)])
    ...
    def swipe(self, x1, y1, x2, y2):
        ...
        if self.deviceId:
            self.cmdShell(
                [self.adbExe, "-s", self.deviceId, "shell", "input", "swipe", str(x1), str(y1), str(x2), str(y2)])
        else:
...
class Element(object):
    ...
    def configureEnglishQwertyLowerKeys(self, refreshDump=True):
        ...
        self.effectiveScreensize = {'width': indusKeyBoardOptionElement.bounds[2],
                                    'height': indusKeyBoardOptionElement.bounds[3]}
        ...
        keys[asdRow[0]] = {'x': keyboard['location']['x'], 'y': keyboard['location']['y'] + (keyHeight * 2),
                           'height': keyHeight, 'width': (keyWidth * 1.5)}
        keys[asdRow[-1]] = {'x': keys[asdRow[-2]]['x'] + keys[asdRow[-2]]['width'],
                            'y': keyboard['location']['y'] + (keyHeight * 2), 'height': keyHeight,
                            'width': (keyWidth * 1.5)}
        ...
        keys['case'] = {'x': keys[asdRow[0]]['x'], 'y': keys[asdRow[0]]['y'] + keyHeight, 'height': keyHeight,
                        'width': (keyWidth * 1.5)}
        keys['backspace'] = {'x': keys[asdRow[-1]]['x'], 'y': keys[asdRow[-1]]['y'] + keyHeight, 'height': keyHeight,
                             'width': (keyWidth * 1.5)}

        # bottom row

        keys['symbol'] = {'x': keys[asdRow[0]]['x'], 'y': keys[asdRow[0]]['y'] + (keyHeight * 2), 'height': keyHeight,
                          'width': (keyWidth * 1.5)}
        keys['regional'] = {'x': keys[asdRow[1]]['x'], 'y': keys[asdRow[1]]['y'] + (keyHeight * 2), 'height': keyHeight,
                            'width': keyWidth}
        keys['.'] = {'x': keys[asdRow[-3]]['x'], 'y': keys[asdRow[-3]]['y'] + (keyHeight * 2), 'height': keyHeight,
                     'width': keyWidth}
        keys[','] = {'x': keys[asdRow[-2]]['x'], 'y': keys[asdRow[-2]]['y'] + (keyHeight * 2), 'height': keyHeight,
                     'width': keyWidth}
        keys['enter'] = {'x': keys[asdRow[-1]]['x'], 'y': keys[asdRow[-1]]['y'] + (keyHeight * 2), 'height': keyHeight,
                         'width': (keyWidth * 1.5)}

        keys['spacebar'] = {'x': (keyWidth * 2.5), 'y': keys['enter']['y'], 'height': keyHeight,
                            'width': (keyWidth * 6)}
    ...
    def click(self, refreshDump=False):
        self.adb.click((self.bounds[0] + self.bounds[2]) / 2, (self.bounds[1] + self.bounds[3]) / 2,
    ...
    def long_press(self, refreshDump=False):
        self.adb.long_press((self.bounds[0] + self.bounds[2]) / 2, (self.bounds[1] + self.bounds[3]) / 2,
                            refreshDump=refreshDump)
        if Cache.currentUIDumpText != Cache.previousUIDumpText:
            self.__reInit(Cache.currentUIDumpText, deviceId=self.deviceId, adbExe=self.adb.adbExe)
        return self
    ...
    def typeTextByKeyboard(self, text):
        for ch in text:
            ...
            if ch.istitle():
                print("KeyPress: Case")
                self.adb.click(self.keyboards['en_qwerty_lower']['case']['x'] + (
                            self.keyboards['en_qwerty_lower']['case']['width'] / 2),
                               self.keyboards['en_qwerty_lower']['case']['y'] + (
                                           self.keyboards['en_qwerty_lower']['case']['height'] / 2))

            print("KeyPress: ", key)
            self.adb.click(
                self.keyboards['en_qwerty_lower'][key]['x'] + (self.keyboards['en_qwerty_lower'][key]['width'] / 2),
                self.keyboards['en_qwerty_lower'][key]['y'] + (self.keyboards['en_qwerty_lower'][key]['height'] / 2))
