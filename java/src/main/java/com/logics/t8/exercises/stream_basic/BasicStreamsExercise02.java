...
//        return categories...;
        ...
        // got each category product list
//        Stream<Product> l1 = categories.flatMap(category -> productsByCategory.get(category).stream());

        /*return categories.flatMap(category -> productsByCategory.get(category).stream())
                  .map(Product::getName)
                  .collect(Collectors.toList());*/
